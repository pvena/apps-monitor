...
import com.example.loginuse.soap.SoapSynchronizeFileSend;
import com.example.loginuse.util.Compress;

public class LogCommandSynchronize extends LogCommand {
	...
	public boolean internalExecute(){
		try {			
			...
			if(compress.compressOK()){
				...
				new SoapSynchronizeFileSend(file,compress).execute();
			}
			else
					return false;
		} catch (Exception ex) {
